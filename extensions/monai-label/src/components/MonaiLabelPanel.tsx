...
export default class MonaiLabelPanel extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      action: {},
      options: {},
      selectedSegmentationModel: MonaiLabelPanel.PREDEFINED_SEGMENTATION_MODELS[0],
    };
  }

  /** Predefined models offered for automatic segmentation in the main panel widget */
  static PREDEFINED_SEGMENTATION_MODELS = [
    'segmentation_spleen',
  ...
  onTriggerDeploy = async () => {
    const nid = this.notification.show({
      title: 'MONAI Label',
      message: 'Triggering MONAI Deploy...',
      type: 'info',
      duration: 2000,
    });

    try {
      const response = await this.client().triggerDeploy();
      hideNotification(nid, this.notification);
      if (response && response.status === 200) {
        this.notification.show({
          title: 'MONAI Label',
          message: 'MONAI Deploy triggered successfully',
          type: 'success',
          duration: 3000,
        });
      } else {
        const errMsg = response?.data?.detail || response?.message || 'Deploy request failed';
        this.notification.show({
          title: 'MONAI Label',
          message: errMsg,
          type: 'error',
      ...
      this.notification.show({
        title: 'MONAI Label',
        message: err?.message || 'Failed to trigger MONAI Deploy',
        type: 'error',
        duration: 5000,
      });
    }
  ...
  onChatSendMessage = async (message: string): Promise<string> => {
  ...
  render() {
    ...
    return (
      <div className="monaiLabelPanel">
        ...
        <div className="monaiLabelPanel-modelSelector">
          <label className="monaiLabelPanel-modelSelectorLabel">
            Automatic segmentation model:
          </label>
          <select
            className="monaiLabelPanel-modelSelect"
            value={this.state.selectedSegmentationModel}
            onChange={(e) =>
              this.setState({ selectedSegmentationModel: e.target.value })
            }
          >
            {MonaiLabelPanel.PREDEFINED_SEGMENTATION_MODELS.map((model) => (
        ...
        <div className="monaiLabelPanel-triggerDeploy">
          <button
            type="button"
            className="actionButton"
            onClick={this.onTriggerDeploy}
          >
            Run Automatic Segmentation
          </button>
        </div>
        <ChatBox
          onSendMessage={this.onChatSendMessage}
          placeholder="Type a message to the MAIA Radiology Assistant..."
