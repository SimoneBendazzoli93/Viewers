...
export default class MonaiLabelClient {
  ...
  async triggerDeploy(params = {}) {
    const url = new URL('deploy/', this.server_url).toString();
  ...
  /**
   * Send a message to an LLM chat endpoint and return the assistant reply text.
   * Expects POST to chat/ with JSON body { message: string }, response { reply: string } or similar.
   */
  async chat(message) {
    const url = new URL('chat/', this.server_url).toString();
    const response = await MonaiLabelClient.api_post(
      url,
      { message },
      null,
      false,
    ...
    if (response && response.status === 200 && response.data) {
      return response.data.reply ?? response.data.response ?? response.data.text ?? JSON.stringify(response.data);
    }
    throw new Error(response?.data?.detail || response?.message || 'Chat request failed');
  }
